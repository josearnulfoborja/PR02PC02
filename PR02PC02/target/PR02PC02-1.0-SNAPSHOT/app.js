...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnRegistrar').onclick = () => {
        ...
        if (!Usuario.validarApellido(apellido)) {
            ...
            errores.push('Apellido inválido: solo letras y espacios.');
            valido = false;
        }
        ...
        if (!Usuario.validarClave(clave, clave2)) {
            ...
            errores.push('Las contraseñas no coinciden o son muy cortas (mínimo 6 caracteres).');
            valido = false;
        }
        ...
        if (!valido) {
            resultado.innerHTML = '<ul style="color:red">' + errores.map(e => '<li>' + e + '</li>').join('') + '</ul>';
            return;
        }
